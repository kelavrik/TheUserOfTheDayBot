...
## LLM-генерация suspense-фраз

Промежуточные «нагнетающие» фразы в `/run` и `/pidor` (между «ВНИМАНИЕ 🔥» и
объявлением победителя) могут генерироваться через LLM — каждый раунд свежие,
но с тем же ироничным смыслом, что у хардкод-набора.

* Провайдер — [OpenRouter](https://openrouter.ai). Модель задаётся
  `OPENROUTER_MODEL` (по умолчанию `google/gemini-2.5-flash-lite`).
* Цепочки генерируются **в фоне на старте бота** и **раз в сутки в 00:00**
  `BOT_TIMEZONE`, кладутся в память. `/run` и `/pidor` берут готовый набор —
  никакой задержки в момент команды.
* Первая строка (`🎉 Сегодня пидор 🌈 дня — `, с именем победителя) и весь
  остальной поток сообщений **не** трогаются LLM — меняются только
  suspense-строки в середине.
* Если `OPENROUTER_API_KEY` не задан, запрос упал или вернул мусор — бот
  молча использует встроенный хардкод-набор. Фича полностью опциональна.

## Годовая церемония

Ровно в 00:00:00 1 января (по `BOT_TIMEZONE`) бот проходит по всем чатам, в
которых он играл, и в каждом проводит:

1. Поздравление с Новым Годом 🎄
2. Анимированное объявление **«Красавчик года YYYY»** — игрок с
   максимальным `user_day_counter` за прошедший год. Suspense-сообщения
   с задержкой 1500 мс между ними (как у `/run`), затем заголовок и имя.
3. Анимированное объявление **«Пидор года YYYY»** — аналогично, по
   `loser_counter`.
...
Если за год никто ни разу не выигрывал данную игру — соответствующая
анимация пропускается (никаких пустых объявлений).

Планировщик — daemon-поток `ScheduledExecutorService`, который
перезапланируется на следующий год сразу после исполнения. Анимация
работает через `Thread.sleep` под per-chat локом, так что одновременный
`/run` или `/pidor` в момент церемонии встанет в очередь, а не пробьётся
с частично сброшенным состоянием. Если бот был выключен ровно в момент
полуночи — церемония за этот год пропустится (возобновится в следующем).
