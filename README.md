...
Ровно в 00:00:00 1 января (по `BOT_TIMEZONE`) бот проходит по всем чатам, в
которых он играл, и в каждом отправляет:

1. Поздравление с Новым Годом 🎄
2. Топ «Красавчик года» (если кто-то выигрывал `/run`)
3. Топ «Пидор года» (если кто-то выигрывал `/pidor`)
4. Сообщение о старте нового раунда — счётчики обнулены 🎊

После этого `chat_user.user_day_counter` и `chat_user.loser_counter`
сбрасываются в `0`, кэш сегодняшнего победителя в `chats` тоже очищается —
ближайший `/run` и `/pidor` 1 января сделают свежий выбор.

Планировщик — daemon-поток `ScheduledExecutorService`, который
перезапланируется на следующий год сразу после исполнения. Если бот был
выключен ровно в момент полуночи — церемония за этот год пропустится
(возобновится в следующем).
